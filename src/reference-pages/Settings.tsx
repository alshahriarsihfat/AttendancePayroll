  ...
  return (
    <div className="space-y-6 animate-fade">
      <Card className="overflow-hidden border-primary/30">
        <div className="flex items-center gap-2.5 border-b border-primary/20 bg-primary-soft/60 px-5 py-3.5">
          <Icon name="store" size={18} className="text-primary" />
          <div><p className="text-sm font-semibold text-primary">Khan Pharmacy — Time & Pay Rules</p><p className="text-xs text-primary/80">Break allowances, login PINs & leave entitlements.</p></div>
        </div>
      ...
      <Card className="overflow-hidden">
        <div className="flex items-center gap-2.5 border-b border-edge bg-linear-to-r from-primary-soft/80 to-primary-soft/40 px-5 py-3.5">
          <Icon name="receipt" size={18} className="text-primary" />
          <div className="flex-1">
            <p className="text-sm font-semibold text-foreground">Payslip Details</p>
            <p className="text-xs text-muted-foreground">Pharmacy information printed on every pay slip.</p>
          </div>
        </div>
        <div className="p-5">
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-primary/25 bg-primary-soft/60 p-4">
            <p className="text-xs font-medium uppercase tracking-wide text-primary">Staff &amp; Supervisor</p>
            <p className="mt-1 text-sm font-semibold text-primary">Unique username + password</p>
            <p className="mt-1 text-xs text-primary/80">Admin manually assigns a username and password to each staff member from the staff form.</p>
          </div>
        </div>
        <p className="mt-3 flex items-center gap-1.5 text-xs text-faint-foreground"><Icon name="info" size={12} /> Supervisors are created by setting a staff member's role to "Supervisor" — they sign in with their own ID and get a reduced dashboard.</p>
      </Card>
      ...
      <Card className="p-5">
        ...
        <div className="mt-5 space-y-6">
          {grouped.map(([cat, entries]) => (
            <div key={cat}>
              <p className="mb-2.5 text-xs font-semibold uppercase tracking-wider text-faint-foreground">{cat}</p>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      ...
      <Card className="border-rose-200 p-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Restores all staff, sessions, leave & payments to the original seed state.</p>
...
function PinCard({ label, value, tone }: { label: string; value: string; tone: "rose" | "indigo" | "emerald" }) {
  const c = { rose: "border-rose-200 bg-rose-50 text-rose-700", indigo: "border-primary/30 bg-primary-soft text-primary", emerald: "border-emerald-200 bg-emerald-50 text-emerald-700" }[tone];
  return (
  ...
  return (
    <div className="rounded-lg border border-edge p-3">
      <div className="flex items-center justify-between">
        ...
        {!editable && <Icon name="lock" size={13} className="text-faint-foreground" />}
      </div>
      <p className="mb-2 text-xs text-faint-foreground">{entry.description}</p>
      <div className="relative">
        <Input value={v} disabled={!editable} onChange={(e) => setV(e.target.value)} onBlur={() => { if (v !== entry.value) onSave(entry.key, v); }} />
        {unit && <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs text-faint-foreground">{unit}</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <Card className="overflow-hidden">
      <div className="flex items-center gap-2.5 border-b border-edge bg-linear-to-r from-primary-soft/80 to-primary-soft/40 px-5 py-3.5">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold text-foreground">Manual Data Editing</p>
          <p className="text-xs text-muted-foreground">Export everything to a file, edit freely, then import it back.</p>
        </div>
      </div>
      <div className="p-5">
        ...
        <div className="mt-4 rounded-lg bg-surface-muted p-3">
          <p className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground"><Icon name="info" size={12} /> How to manually edit anything</p>
          <ol className="ml-4 mt-1.5 list-decimal space-y-1 text-xs text-muted-foreground">
            <li>Click <b>Export Data</b> — a <code className="rounded-md bg-surface-muted px-1 text-[10px] text-foreground">.json</code> file downloads.</li>
          ...
          <p className="mt-2 text-[11px] text-faint-foreground">Tip: you can also edit directly in-app — Staff page (people), Settings (rules), or connect Google Sheets (above) for spreadsheet editing.</p>
        </div>
      </div>
      <ConfirmDialog
        open={confirmImport}
